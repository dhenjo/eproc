      ...
      if ($dh = opendir($dir)){
        while (($file = readdir($dh)) !== false){
          ...
          if($info['extension'] == "php"){
            $cek = $this->global_models->get_field("m_controller", "id_controller", array("link" => $info['filename'],"id_module" => $id_module));
            if($cek < 1){
              $kirim = array(
                "id_module"           => $id_module,
...
//          die;
  ...
  function delete_form($id_form){
    $this->global_models->delete("m_form", array("id_form" => $id_form));
