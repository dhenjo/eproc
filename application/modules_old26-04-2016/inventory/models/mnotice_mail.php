...
//        $pt_product_tour = $this->global_models->get("product_tour_book", array("id_product_tour" => $data['id_product_tour'],"status <" => 4,""));
    ...
    function cancel_product_tour_information($id_information){
     ...
     function notice_tour_leader($data_tl){
       $sql = "SELECT B.title,B.id_store,B.id_store_region,CONCAT(C.first_name,' ',C.last_name) AS name"
        . " ,A.start_date,A.end_date"
        . " ,A.id_product_tour_information,A.kode"  
        . " FROM product_tour_information AS A"
        . " LEFT JOIN product_tour AS B ON A.id_product_tour = B.id_product_tour"
        . " LEFT JOIN product_tour_leader AS C ON A.id_product_tour_information = C.id_product_tour_information" 
        . " WHERE A.id_product_tour_information ='{$data_tl['id_product_tour_information']}'";
      $data = $this->global_models->get_query($sql);
      
      $full_name = $data_tl['first_name']." ".$data_tl['last_name'];
      
      $l_store  = $this->global_models->get_field("store_region", "title", array("id_store_region" => $data[0]->id_store_region));
//      print "<pre>";
//      print_r($data);
//      print "</pre>";  die;
//      print $data[0]->name." 9<br>";
//      print $full_name."7";
//      die;
      if($data[0]->name != $full_name){
        
      $pt_product_tour_info = $this->global_models->get("product_tour_book", array("id_product_tour_information" => $data_tl['id_product_tour_information'],"status <" => 4));
       ...
       $tanggal = date("d-m-Y", strtotime($data[0]->start_date))." - ".date("d-m-Y", strtotime($data[0]->end_date));
        ...
        $subject_email = "Notifikasi Tour Leader";
        if($data_tl['status_perubahan'] == 2){
          
          $isi_email = "<html>
            <body>
              Dear Bookers Group tour Status<br><br>
             
                Team tour Operation <b>{$l_store}</b> telah update tour leader {$full_name} untuk tour {$data[0]->title} pada tanggal keberangkatan: {$tanggal}<br>
          ...
          $isi_email = "<html>
            <body>
              Dear Bookers Group tour Status<br><br>
                ...
                Team tour Operation <b>{$l_store}</b> telah menunjuk tour leader {$full_name} untuk tour {$data[0]->title} pada tanggal keberangkatan: {$tanggal}<br>
