  ...
  <div class="col-xs-12">
    ...
    <div class="nav-tabs-custom">
        <ul class="nav nav-tabs">
             <li class="active"><a href="#view-cetakan-invoice" data-toggle="tab"><b>List Cetakan Invoice</b></a></li>
            <?php
            if($list[0]->status < 6 OR $list[0]->status == 11 ){
            if($list[0]->id_mrp_request){
            ?>
            <li><a href="#cetakan-invoice" id="dt-cetakan-invoice" data-toggle="tab"><b>Cetakan Invoice</b></a></li>
            <?php } }
        ...
        <div class="tab-content no-padding">
             <div class="chart tab-pane active" id="view-cetakan-invoice" style="position: relative; height: 300px;">
                <?php $this->load->view("mrp-request/pengadaan-cetakan-invoice/request-cetakan-invoice/view-request-pengadaan-cetakan-invoice"); ?>
            </div>
           <?php 
           if($list[0]->status < 6 OR $list[0]->status == 11){
           if($list[0]->id_mrp_request){
           ?>
           <div class="chart tab-pane " id="cetakan-invoice" style="position: relative; height: 300px;">
                 <?php $this->load->view('mrp-request/pengadaan-cetakan-invoice/request-cetakan-invoice/add-request-pengadaan-cetakan-invoice');?>
            </div>
           <?php } }
         if($this->session->userdata("id") == 1 OR $this->nbscache->get_olahan("permission", $this->session->userdata("id_privilege"), "request-mutasi-stock", "edit") !== FALSE){         
            if($this->session->userdata("id") == 1 OR $list[0]->status >= 3){
