          ...
          $this->session->set_userdata($set);
           ...
           $hr_pegawai = $this->global_models->get("hr_pegawai", array("id_users" => $this->session->userdata("id")));
            if($this->session->userdata("asset_dept_search_id_company")){
                $id_company = $this->session->userdata("asset_dept_search_id_company");
            }else{
                $id_company = $hr_pegawai[0]->id_hr_company;
            }
            
            if($this->session->userdata("asset_dept_search_id_hr_master")){
                $id_dt_master_organisasi = $this->session->userdata("asset_dept_search_id_hr_master");
            }else{
                $id_dt_master_organisasi = $hr_pegawai[0]->id_hr_master_organisasi;
            }
            
           $set = array(
            "asset_dept_search_id_company"          => $id_company,
            "asset_dept_search_id_hr_master"        => $id_dt_master_organisasi,  
        );
          $this->session->set_userdata($set);
       }
