...
class Mrp_setting_ajax extends MX_Controller {
  function __construct() {
   ...
   function get_alert_email($start = 0){
        
    $where = "WHERE A.status =1";

     $data = $this->global_models->get_query("SELECT A.struktural,A.id_alert_email_struktural"
        . " FROM alert_email_struktural AS A"
        . " {$where}"
        . " ORDER BY A.id_alert_email_struktural DESC"
        . " LIMIT {$start}, 10"
        );
    ...
    foreach ($data AS $ky => $da){
         ...
         $btn2 = "<div class='btn-group'>"
               . "<button type='button' class='btn btn-info dropdown-toggle' data-toggle='dropdown'>"
                ...
                . "<li><a href='".site_url("mrp/mrp-setting/add-alert-email/{$da->id_alert_email_struktural}")."' title='Edit'>Edit</a></li>"
                . "<li class='divider'></li>"
                . "<li><a href='".site_url("mrp/mrp-setting/add-alert-email-users/{$da->id_alert_email_struktural}")."' title='Users'>Users</a></li>"
                . "<li><a href='".site_url("mrp/mrp-setting/add-alert-email-approval/{$da->id_alert_email_struktural}")."' title='Approval'>Approval</a></li>"       
                . "</ul>"
                 . "</div>";
...
//    $this->debug($return, true);
  ...
  function get_alert_email_procurement($start = 0){
        
    $where = "WHERE A.status =1";

     $data = $this->global_models->get_query("SELECT A.id_alert_email_procurement,B.title"
        . " FROM alert_email_procurement AS A"
        . " LEFT JOIN mrp_type_inventory AS B ON A.id_mrp_type_inventory = B.id_mrp_type_inventory"
        . " {$where}"
        . " ORDER BY A.id_alert_email_procurement DESC"
        . " LIMIT {$start}, 10"
        );
//    print  $this->db->last_query(); die;
//    $data_array = json_decode($data);
//    $this->debug($data, true);
    ...
    foreach ($data AS $ky => $da){
        $dd = $this->global_models->get_query("SELECT B.name "
                . " FROM alert_email_procurement_users AS A"
                . " LEFT JOIN m_users AS B ON A.id_users = B.id_users"
                . " WHERE A.id_alert_email_procurement='{$da->id_alert_email_procurement}' ");
         $no[$ky]= 0;
         $name = "";
        foreach ($dd as $key => $value){
         ...
         $btn2 = "<div class='btn-group'>"
               . "<button type='button' class='btn btn-info dropdown-toggle' data-toggle='dropdown'>"
                ...
                . "</button>"
                . "<ul class='dropdown-menu' role='menu'>"
                . "<li><a href='".site_url("mrp/mrp-setting/add_alert_email_procurement/{$da->id_alert_email_procurement}")."' title='Edit'>Edit</a></li>"
                . "</ul>"
                . "</div>";
...
function get_setting_notification_email($start = 0){
        
    $where = "WHERE A.status =1";

     $data = $this->global_models->get_query("SELECT A.id_setting_blast_email_rg,A.id_mrp_supplier,B.name AS supplier,A.days"
        . " FROM setting_blast_email_rg AS A"
        . " LEFT JOIN mrp_supplier AS B ON A.id_mrp_supplier = B.id_mrp_supplier"
        . " {$where}"
        . " ORDER BY A.id_setting_blast_email_rg DESC"
        . " LIMIT {$start}, 10"
        );
//    print  $this->db->last_query(); die;
//    $data_array = json_decode($data);
//    $this->debug($data, true);
    ...
    foreach ($data AS $ky => $da){
       $hasil[] = array(
                ...
                $da->days,
                "<div class='btn-group'>"
                  . "<a href='".site_url("mrp/mrp-setting/add-setting-notification-email-rg/{$da->id_setting_blast_email_rg}")."' type='button' title='Edit Setting Notification' class='btn btn-info btn-flat' style='width: 40px'><i class='fa fa-edit'></i></a>"
                  . "<a href='".site_url("mrp/mrp-setting/delete-setting-notification/{$da->id_setting_blast_email_rg}")."' type='button' title='Delete Setting Notification' class='btn btn-danger btn-flat' style='width: 40px'><i class='fa fa-trash-o'></i></a>"              
                . "</div>"
              );
...
//    $where = "WHERE {$qry}";
     ...
     $data = $this->global_models->get_query("SELECT A.id_mrp_setting_users,A.title,B.name,D.title AS name_organisasi,E.code "
...
//    $this->debug($data, true);
    ...
    foreach ($data AS $ky => $da){
       $hasil[$ky] = array(
                $da->title,
                $da->name." [".$da->name_organisasi."]"."[".$da->code."]",
                "<div class='btn-group'>"
...
//    $this->debug($return, true);
  ...
  function get_users(){
    ...
    $items = $this->global_models->get_query("
      ...
      WHERE 
      A.id_users > 1 AND (LOWER(A.name) LIKE '%{$q}%' OR LOWER(C.title) LIKE '%{$q}%' OR LOWER(A.email) LIKE '%{$q}%')
      LIMIT 0,10
      ");
