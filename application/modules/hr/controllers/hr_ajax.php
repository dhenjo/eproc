...
class Hr_ajax extends MX_Controller {
  ...
  function get_hr_master_pegawai($start = 0){
    ...
    $data = $this->global_models->get_query("SELECT  A.nip,A.id_hr_pegawai,B.name,B.email,C.title AS nama_company,E.title AS nama_department"
...
//    $this->debug($data, true);
    ...
    foreach ($data AS $da){
         ...
         $sign = "<a href='".site_url("hr/hr-master/pegawai-signature/{$da->id_hr_pegawai}")."' type='button' title='signature' class='btn btn-warning btn-flat' style='width: 40px'><i class='fa fa-key'></i></a>".
                 "<a href='".site_url("hr/hr-master/delete-hr-pegawai/{$da->id_hr_pegawai}")."' type='button' title='signature' class='btn btn-danger btn-flat' style='width: 40px'><i class='fa fa-times'></i></a>";
         }
      $hasil[] = array(
        $da->name."<br>".$da->email,
        $da->nama_company,
...
//    $this->debug($return, true);
   ...
   function get_hr_master_mo_divisi($start = 0){
...
//    $this->debug($data, true);
    ...
    foreach ($data AS $da){
         if($this->session->userdata("id") == 1){
        $btn_del = "<a href='".site_url("hr/hr-master/delete-master-organisasi/{$da->id_hr_master_organisasi}/mo-divisi")."' type='button' class='btn btn-danger btn-flat' style='width: 40px'><i class='fa fa-trash-o'></i></a>";
    }
      $hasil[] = array(
        $da->title,
        $da->code,
        $da->direktorat,
        "<div class='btn-group'>"
          . "<a href='".site_url("hr/hr-master/add-mo-divisi/{$da->id_hr_master_organisasi}")."' type='button' class='btn btn-info btn-flat' style='width: 40px'><i class='fa fa-edit'></i></a>"
          . $btn_del
        . "</div>"
      );
    }
...
//    $this->debug($return, true);
  ...
  function get_hr_master_dt_department($start = 0){
      
    $data = $this->global_models->get_query("SELECT  A.id_hr_master_organisasi,A.title,A.code,B.title AS divisi,C.title AS direktorat"
        . " FROM hr_master_organisasi AS A"
        . " LEFT JOIN hr_master_organisasi AS B ON A.parent = B.id_hr_master_organisasi"
        . " LEFT JOIN hr_master_organisasi AS C ON B.parent = C.id_hr_master_organisasi"   
        . " WHERE A.level=3"
...
//    $this->debug($data, true);
    ...
    foreach ($data AS $da){
         if($this->session->userdata("id") == 1){
        $btn_del = "<a href='".site_url("hr/hr-master/delete-master-organisasi/{$da->id_hr_master_organisasi}/dt-department")."' type='button' class='btn btn-danger btn-flat' style='width: 40px'><i class='fa fa-trash-o'></i></a>";
        }
      $hasil[] = array(
        $da->title,
        $da->code,
        ...
        $da->divisi,  
        "<div class='btn-group'>"
          . "<a href='".site_url("hr/hr-master/add-dt-department/{$da->id_hr_master_organisasi}")."' type='button' class='btn btn-info btn-flat' style='width: 40px'><i class='fa fa-edit'></i></a>"
          . $btn_del
          . "</div>"
      );
    }
