...
class Hr_master extends MX_Controller {
  ...
  function delete_hr_pegawai($id_hr_pegawai){
   $kirim = array(
            "status"                        => 2,
    ...
    $this->global_models->update("hr_pegawai", array("id_hr_pegawai" => $id_hr_pegawai),$kirim);
  ...
  function company(){
    ...
    $detail = $this->global_models->get_query("SELECT A.title,A.telp,A.fax,A.office,A.address,B.title"
    . " FROM hr_company AS A"
    . " LEFT JOIN hr_master_organisasi AS B ON A.storage_pusat = B.id_hr_master_organisasi"
    );
...
//          . "'iDisplayLength': 20"
      ...
      . 'function ambil_data(table, mulai){'
        . '$.post("'.site_url("hr/hr-ajax/get-hr-master-mo-divisi").'/"+mulai, function(data){'
          . '$("#loader-page").show();'
          . 'var hasil = $.parseJSON(data);'
          . 'if(hasil.status == 2){'
            . 'table.fnAddData(hasil.hasil);'
...
//            . "$('#script-tambahan').html(data);"
    ...
    $this->template->build('hr-master/mo-divisi', 
      array(
            'url'         => base_url()."themes/".DEFAULTTHEMES."/",
            'menu'        => "hr/hr-master/mo-divisi",
            'data'        => $list,
            'title'       => lang("hr_master_divisi"),
            'css'         => $css,
            'foot'        => $foot,
            'parent'        => $parent,
//            'company'     => $dropdown_company,
...
//          . "'iDisplayLength': 20"
      ...
      . 'function ambil_data(table, mulai){'
        . '$.post("'.site_url("hr/hr-ajax/get-hr-master-dt-department").'/"+mulai, function(data){'
          . '$("#loader-page").show();'
          . 'var hasil = $.parseJSON(data);'
          . 'if(hasil.status == 2){'
            . 'table.fnAddData(hasil.hasil);'
...
//            . "$('#script-tambahan').html(data);"
    ...
    $this->template->build('hr-master/dt-department', 
      array(
            'url'         => base_url()."themes/".DEFAULTTHEMES."/",
            'menu'        => "hr/hr-master/dt-department",
            'data'        => $list,
...
//            'users'       => $dropdown_user,
    ...
    $this->template
      ->set_layout('default')
      ->build('hr-master/dt-department');
  }
  
   public function add_mo_divisi($id_hr_master_organisasi = 0){
...
//    $dropdown_section = $this->global_models->get_dropdown("hr_master_organisasi", "id_hr_master_organisasi", "title", TRUE,array("level" => "2")); 
      ...
      $this->template->build("hr-master/add-mo-divisi", 
        array(
              'url'         => base_url()."themes/".DEFAULTTHEMES."/",
              'menu'        => 'hr/hr-master/mo-divisi',
...
//              'section'     => $dropdown_section,
              'breadcrumb'  => array(
                    "master_divisi"  => "hr/hr-master/mo-divisi"
                ),
              'css'         => $css,
              'foot'        => $foot
            ));
...
//      print_r($pst); die;
      ...
      redirect("hr/hr-master/mo-divisi");
    }
  }
  
  public function add_dt_department($id_hr_master_organisasi){
...
//                 ."department(id_dept);"
      ...
      ."var dataString2 = 'id_direktorat='+ id;"
         ...
         ."$.ajax"
            ."({"
            ."type: 'POST',"
            ."url: '".site_url("hr/hr-ajax/ajax-department/{$detail[0]->divisi}")."',"
            ."data: dataString2,"
            ."cache: false,"
            ."success: function(html)"
            ."{"
...
//    $dropdown_department = $this->global_models->get_dropdown("hr_master_organisasi", "id_hr_master_organisasi", "title", TRUE,array("level" => "2")); 
      ...
      $this->template->build("hr-master/add-dt-department", 
        array(
              'url'         => base_url()."themes/".DEFAULTTHEMES."/",
              'menu'        => 'hr/hr-master/dt-department',
...
//              'department'     => $dropdown_department,
              ...
              'breadcrumb'  => array(
                    "master_department"  => "hr/hr-master/dt-department"
                ),
              'css'         => $css,
              'foot'        => $foot
            ));
      $this->template
        ->set_layout('form')
        ->build("hr-master/add-dt-department");
    }
    else{
      $pst = $this->input->post(NULL);
//      print_r($pst); die;
      ...
      redirect("hr/hr-master/dt-department");
    }
  }
    ...
    if(!$this->input->post(NULL)){
        ...
        . ",C.id_hr_master_organisasi AS divisi,D.id_hr_master_organisasi AS direktorat"
        . " FROM hr_master_organisasi AS A"
        . " LEFT JOIN hr_master_organisasi AS B ON A.parent = B.id_hr_master_organisasi"
        . " LEFT JOIN hr_master_organisasi AS C ON B.parent = C.id_hr_master_organisasi"
...
//             . "alert(id_department);"        
      ...
      ."var dataString2 = 'id_department='+ id;"
         ."$.ajax"
            ."({"
            ."type: 'POST',"
            ."url: '".site_url("hr/hr-ajax/ajax-divisi/{$detail[0]->department}")."',"
            ."data: dataString2,"
            ."cache: false,"
            ."success: function(html)"
            ."{"
...
//                . "$('#wadah').insertBefore(data);"
      ...
      foreach ($detail AS $key => $det){
        $hasil .= "<div class='input-group margin' id='direktorat-box{$key}'>"
            ...
            . "<input type='text' class='form-control' value='{$det->id_hr_master_organisasi}' id='id_direktorat{$key}' name='id_direktorat[]' style='display: none'>"
            . "<span class='input-group-btn'>"
              . "<a href='javascript:void(0)' class='btn btn-danger btn-flat delete' isi='direktorat-box{$key}'>"
...
//        print_r($pst); die;
    ...
    public function storage_pusat($id_hr_company){
//       print "aa"; die;
    if(!$this->input->post(NULL)){
      $css = "<link href='".base_url()."themes/".DEFAULTTHEMES."/css/jQueryUI/jquery-ui-1.10.3.custom.min.css' rel='stylesheet' type='text/css' />";
      $foot = "<script src='".base_url()."themes/".DEFAULTTHEMES."/js/jquery.ui.autocomplete.min.js' type='text/javascript'></script>"
...
//      }
//      $detail = $this->global_models->get("hr_company", array("id_hr_company" => $id_hr_company));
      $detail = $this->global_models->get_query("SELECT A.id_hr_company,A.storage_pusat,B.title,B.code"
    . " FROM hr_company AS A"
    . " LEFT JOIN hr_master_organisasi AS B ON A.storage_pusat = B.id_hr_master_organisasi"
    . " WHERE A.id_hr_company='{$id_hr_company}'"
    );
      $this->template->build("hr-master/storage-pusat", 
        array(
              'url'         => base_url()."themes/".DEFAULTTHEMES."/",
              'menu'        => 'hr/hr-master/company',
              'title'       => lang("List Storage Pusat"),
              'detail'      => $detail,
              'breadcrumb'  => array(
                    "Company"  => "hr/hr-master/company"
                ),
...
//      foreach ($pst['id_direktorat'] as $value) {
          $kirim = array(
             "storage_pusat"                 => $pst["id_direktorat"], 
             "update_by_users"               => $this->session->userdata("id"),
             "update_date"                   => date("Y-m-d H:i:s"),
          );
//      }
//          print_r($kirim);
//          die;
      if($this->global_models->update("hr_company",array("id_hr_company" =>$pst["id_detail"]), $kirim)){
        $this->session->set_flashdata('success', 'Data tersimpan');
      }
      else{
        $this->session->set_flashdata('notice', 'Data tidak tersimpan');
...
//            . "$('#script-tambahan').html(data);"
  ...
  public function add_pegawai($id_hr_pegawai = 0){
    ...
    if(!$this->input->post(NULL)){
//      $detail = $this->global_models->get("hr_pegawai", array("id_hr_pegawai" => $id_hr_pegawai));
        $where = "WHERE A.status=1 AND id_hr_pegawai='{$id_hr_pegawai}'";
        $detail = $this->global_models->get_query("SELECT  A.nip,A.id_hr_pegawai,A.id_users,A.id_hr_company,B.name,B.email,C.title AS nama_company"
