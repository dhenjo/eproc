...
//      $detail = $this->global_models->get_query("mrp_supplier_inventory", array("id_mrp_supplier" => $id_mrp_supplier,"id_mrp_supplier_inventory" => $id_mrp_supplier_inventory));
        ...
        . ",E.code AS type,D.title AS brand,F.title AS satuan,A.jenis"
...
//                'detail'            => $data,           
      ...
      if($pst['id_detail']){
          ...
          if($dt_update[0]->harga != str_replace(",","",$pst['harga']) OR $dt_update[0]->kategori_inventory != $pst['kategori_inventory'] 
                  OR $dt_update[0]->id_mrp_inventory != $pst['id_mrp_inventory'] OR $dt_update[0]->status != $status){
               $kirim = array(
