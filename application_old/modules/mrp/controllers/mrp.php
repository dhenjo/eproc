    ...
    $foot .= "<script>"
        ...
        . "$(function() {"
            . "$( '#users' ).autocomplete({"
                  ...
                  . "source: '".site_url("mrp/mrp-ajax/get-pegawai/{$this->session->userdata("id")}/{$dt_status}")."',"
                  . "minLength: 1,"
                  . "search  : function(){ $(this).addClass('working');},"
                  . "open    : function(){ $(this).removeClass('working');},"
                  . "select: function( event, ui ) {"
...
//            . "alert(aData);"
      ...
      . "$('#purchase-orders2').click(function(){"
       ...
       . 'function ambil_data(table, mulai){'
        . '$.post("'.site_url("mrp/mrp-ajax/get-task-mrp-request-pengadaan").'/"+mulai, function(data){'
          . '$("#loader-page").show();'
          . 'var hasil = $.parseJSON(data);'
          . 'if(hasil.status == 2){'
            . 'table.fnAddData(hasil.hasil);'
...
//            . "$('#script-tambahan').html(data);"
      ...
      . 'function ambil_data3(table, mulai){'
        . '$.post("'.site_url("mrp/mrp-ajax/get-task-orders-request-asset/1/{$id_mrp_task_orders}").'/"+mulai, function(data){'
          . '$("#loader-page3").show();'
          . 'var hasil = $.parseJSON(data);'
          . 'if(hasil.status == 2){'
            . 'table.fnAddData(hasil.hasil);'
...
//            . "$('#script-tambahan').html(data);"
      ...
      . 'function ambil_data4(table, mulai){'
        . '$.post("'.site_url("mrp/mrp-ajax/get-task-orders-request-asset/2/{$id_mrp_task_orders}").'/"+mulai, function(data){'
          . '$("#loader-page4").show();'
          . 'var hasil = $.parseJSON(data);'
          . 'if(hasil.status == 2){'
            . 'table.fnAddData(hasil.hasil);'
...
//        print_r($data); die;
     $this->template->build("main-mrp/update-po", 
        array(
              'url'         => base_url()."themes/".DEFAULTTHEMES."/",
              'menu'        => 'mrp/task-orders',
              'title'       => lang("mrp_add_po"),
...
//      print_r($pst); die;
   ...
   public function detail_po($id_mrp_task_orders = 0,$id_mrp_po = 0){
    ...
    $list = $this->global_models->get_query("SELECT A.id_mrp_po,A.no_po,A.code,A.status,A.note,A.tanggal_dikirim,A.status,A.tanggal_payment,A.note_payment,A.status_payment"
...
//     $id_mrp_task_orders = 0,$id_mrp_po 
    ...
    $this->template->build("main-mrp/detail-po", 
        array(
              'url'                 => base_url()."themes/".DEFAULTTHEMES."/",
              'menu'                => 'mrp/list-po',
  ...
  <br>Mohon dapat dikirimkan kepada kami '.$tgl.' barang-barang sbb :'
        ...
        . "</table><br><br>";
         ...
         if($detail[0]->signature){
            $sgn = 'src='.base_url().'files/antavaya/signature/'.$detail[0]->signature;
...
//     $id_mrp_task_orders = 0,$id_mrp_po 
    ...
    $this->template->build("main-mrp/rg", 
        array(
              'url'                 => base_url()."themes/".DEFAULTTHEMES."/",
              'menu'                => 'mrp/list-po',
              'title'               => lang("Receiving Goods"),
...
//       );
    ...
    $css = "<link href='".base_url()."themes/".DEFAULTTHEMES."/css/datatables/dataTables.bootstrap.css' rel='stylesheet' type='text/css' />"
      ...
      $url2 = base_url("mrp/rg-department/{$id_mrp_receiving_goods_po}");
    $foot = ""
      . "<script src='".base_url()."themes/".DEFAULTTHEMES."/js/plugins/datatables/jquery.dataTables.js' type='text/javascript'></script>"
      . "<script src='".base_url()."themes/".DEFAULTTHEMES."/js/plugins/datatables/dataTables.bootstrap.js' type='text/javascript'></script>"
     ;
