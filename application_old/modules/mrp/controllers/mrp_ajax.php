...
//    $this->debug($data, true);
    ...
    foreach ($data AS $da){
      $hasil[] = array(
        ...
        $da->address,  
        "<div class='btn-group'>"
          . "<a href='".site_url("mrp/mrp-master/add-supplier/{$da->id_mrp_supplier}")."' type='button' class='btn btn-info btn-flat' style='width: 40px'><i class='fa fa-edit'></i></a>"
           . "<a href='".site_url("mrp/mrp-master/supplier-inventory/{$da->id_mrp_supplier}")."' type='button' class='btn btn-info btn-flat' style='width: 40px'><i class='fa fa-list-alt'></i></a>"       
        . "</div>"
      );
    }
    $return['hasil'] = $hasil;
...
//    $this->debug($data, true);
    ...
    foreach ($data AS $key => $da){
      ...
      $hasil[] = array(
        ...
        $status[$da->status],   
        "<div class='btn-group'>"
          . "<a href='".site_url("mrp/mrp-master/add-supplier-inventory/{$da->id_mrp_supplier}/{$da->id_mrp_supplier_inventory}")."' type='button' class='btn btn-info btn-flat' style='width: 40px'><i class='fa fa-edit'></i></a>"
          . "<a href='".site_url("mrp/mrp-master/history-supplier-inventory/{$da->id_mrp_supplier}/{$da->id_mrp_supplier_inventory}")."' type='button' class='btn btn-info btn-flat' style='width: 40px'><i class='fa fa-columns'></i></a>"       
        . "</div>"
      );
    }
    $return['hasil'] = $hasil;
...
//        . ",D.code AS department,F.title AS perusahaan,H.code,H.id_mrp_task_orders"
    ...
    foreach ($data AS $ky => $da){
       if($da->status == 1){
           if($da->create_by_users == $id_users OR $id_users == 1){
               $hasil[$ky] = array(
                date('d-M-Y H:i:s', strtotime($da->create_date))."<br>[".$type_inventory[$da->type_inventory]."]<br>Kode Request:<b>".$da->ro_code."</b>",  
...
//    $jenis = array(1 => "Habis Pakai", 2 => "Asset");
    $status = array( 1=> "<span class='label bg-orange'>Create</span>", 2 => "<span class='label bg-orange'>Draft</span>",
        3 => "<span class='label bg-green'>Approved</span>",4 => "<span class='label bg-green'>Task Order</span>",
        5 => "<span class='label bg-green'>Proses PO</span>",6 => "<span class='label bg-green'>Sent PO</span>",9 => "<span class='label bg-green'>Closed Request Orders</span>");
//    $status = array(1 => "Create", 2 => "Approve");
    $type_inventory = array(1 => "Pengadaan Cetakan", 2 => "Pengadaan ATK");
        $no = 0;
       $hide = 0;
...
//    $this->debug($return, true);
  ...
  function get_task_mrp_request_pengadaan($start = 0){
...
//    $status = array(1 => "Create", 2 => "Approve");
    foreach ($data AS $da){
      ...
      $hasil[] = array(
        "<a data-toggle='collapse' data-parent='#accordion'  href='#collapseTwo' onclick='coba_data2({$da->type_inventory},{$da->id_mrp_request});'>".$da->req_code."</a>"
                . "<script>"
        ...
        . "</script>",
        $type[$da->type_inventory],  
        "<a href='".site_url("mrp/add-request-pengadaan-{$dt_link}/{$da->id_mrp_request}")."'>".$da->nama_pegawai."<br>".$da->nip."</a>",
        $da->perusahaan."<br>Department:".$da->department,
        $da->note,
        $status[$da->status], 
        "<div class='btn-group'>"
...
//    $this->debug($return, true);
  ...
  function get_task_orders_request_asset($status = 0,$id_mrp_task_orders = 0,$start = 0){
    ...
    $data = $this->global_models->get_query("SELECT A.jumlah,D.code AS brand,C.name AS nama_barang"
            . ",B.title AS title_spesifik,E.title AS satuan"
...
//    $status = array(1 => "Create", 2 => "Approve");
    ...
    foreach ($data AS $da){
      $hasil[] = array(
        $da->nama_barang." ".$da->title_spesifik,
        $da->brand,
        $da->satuan,
        $da->jumlah,
      );
...
function update_rg($id_mrp_receiving_goods_po = 0){
  $id_mrp_receiving_goods_po_asset = $_POST['id_mrp_receiving_goods_po_asset'];
  $rg =  $_POST['rg'];
...
//  $dt_status = array("5" => "Approve PO","4" => "Pengajuan PO", "3" => "Draft");
 ...
 foreach ($arr_id as $ky => $val) {
   ...
   if($jmlh_now >= 0){
    ...
    $this->mmrp->mutasi_rg($id_mrp_stock_in,$get_task_order_asset[$ky][0]->id_mrp_receiving_goods_po,$get_task_order_asset[$ky][0]->id_mrp_inventory_spesifik);
...
//  $dt_status = array("5" => "Approve PO","4" => "Pengajuan PO", "3" => "Draft");
 ...
 foreach ($arr_id as $ky => $val) {
   ...
   if($jmlh_now >= 0){
    ...
    $this->load->model('mrp/mmrp');
    $this->mmrp->mutasi_rg($id_mrp_stock_in,$get_task_order_asset[$ky][0]->id_mrp_receiving_goods_po,$get_task_order_asset[$ky][0]->id_mrp_inventory_spesifik,$arr_id_mrp_request[$ky]);
...
//    $status = array(1 => "Create", 2 => "Approve");
    ...
    if($da->status <= "3"){
        ...
        if($da->status >= 6){
            $btn_status_payment = "<a href='".site_url("mrp/delete-list-po/{$da->id_mrp_po}")."' type='button' class='btn btn-warning btn-flat' title='Payment Request' style='width: 40px'><i class='fa fa-indent'></i></a>";
        }
        ...
        if($da->status >= 6){
            $btn_status_payment = "<a href='".site_url("mrp/status-payment/{$da->id_mrp_po}")."' type='button' class='btn btn-warning btn-flat' title='Payment Request' style='width: 40px'><i class='fa fa-indent'></i></a>";
        }
            ...
            "<div class='btn-group'>"
              . $btn_update
              . "<a href='".site_url("mrp/detail-po/{$da->id_mrp_task_orders}/{$da->id_mrp_po}")."' type='button' class='btn btn-info btn-flat' title='List Purchase Order' style='width: 40px'><i class='fa fa-edit'></i></a>"
    //          . $btn_rg
              . $btn_status_payment        
            . "</div>"
          );
...
//    $this->debug($return, true);
  ...
  function get_mrp_list_rg($start = 0){
      ...
      $where = " WHERE A.status =6 OR A.status =7";
...
//    $jenis = array(1 => "Habis Pakai", 2 => "Asset");
    $status = array( 3=> "<span class='label bg-orange'>Draft PO</span>", 4 => "<span class='label bg-green'>Proses Pengajuan PO</span>",
      5 => "<span class='label bg-green'>Approved PO</span>", 6 =>"<span class='label bg-green'>Receiving Goods</span>", 7 =>"<span class='label bg-green'>Closed PO</span>");
...
//            }
//        }
        $where = " WHERE (A.status =6 OR A.status =7) AND H.title IS NOT NULL ";
...
//        . " LIMIT {$start}, 10");
        ...
        $data = $this->global_models->get_query("SELECT F.code,F.id_mrp_request,A.id_mrp_receiving_goods_po,A.status,A.create_date,A.code AS code_rg"
        . ",B.no_po AS code_po,B.id_mrp_po,C.id_mrp_task_orders"
...
//       $url = base_url()."themes/".DEFAULTTHEMES."/img/ajax-loader.gif";
    ...
    $data = $this->global_models->get_query("SELECT A.id_mrp_receiving_goods_po_asset,A.rg,A.jumlah,A.note,"
        . "B.title AS title_spesifik,C.name AS nama_barang,E.title AS satuan"
        . " FROM mrp_receiving_goods_po_asset AS A"
        . " LEFT JOIN mrp_inventory_spesifik AS B ON A.id_mrp_inventory_spesifik = B.id_mrp_inventory_spesifik"
        . " LEFT JOIN mrp_inventory_umum AS C ON B.id_mrp_inventory_umum = C.id_mrp_inventory_umum"
        . " LEFT JOIN mrp_brand AS D ON B.id_mrp_brand = D.id_mrp_brand"
...
//                            . "alert(dt_jml);" 
                    . "var hasil = {$da->jumlah} - dt_jml;"
                    . "if({$da->jumlah} > dt_jml){"
                        . "$('#jml_{$da->id_mrp_receiving_goods_po_asset}').text(jml);"
                    . "}else{"
...
//      $where = "WHERE A.status >= 3 AND A.id_mrp_receiving_goods_po = '$id_mrp_receiving_goods_po'  ";
    ...
    $data = $this->global_models->get_query("SELECT L.id_users,I.id_mrp_request,A.id_mrp_receiving_goods_po_asset,A.jumlah AS jml_po,"
        . "A.note,B.title AS title_spesifik,C.name AS nama_barang,E.title AS satuan,"
...
function get_pegawai($id_users = 0,$status = 0){
...
foreach ($detail as $val) {
    ...
    if($val->E1){
     ...
     $where2 = "WHERE A.id_users='{$id_users}'";
...
//      die;
  ...
  private function olah_task_order_code(&$kode){
    ...
    $kode_random = random_string('alnum', 5);
